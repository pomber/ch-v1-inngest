...
![!screenDevserver Inngest Dev Server](/dev-server.png)
...
<Note>
  Note that you can import serve() for
  other frameworks and the rest of the
  code, in fact, remains the same — only
  the import statement changes (instead of
  inngest/next, it would be inngest/astro,
  inngest/remix, and so on). TODO create
  aside
</Note>
...
![!screenDevserver Inngest Dev Server](/dev-server.png)
![!screenNextjs Inngest Dev Server](/dev-server.png)

### !!steps Send a test event

To send a test event, click on “Test Event” in the top right corner.

![!screenDevserver Inngest Dev Server](/dev-server-frame.png)
...
![!screenDevserver Inngest Dev Server](/dev-server-test-event-console.png)
...
![!screenDevserver Inngest Dev Server](/dev-server-event-1.png)
...
![!screenDevserver Inngest Dev Server](/dev-server-event-details-1.png)
...
![!screenDevserver Inngest Dev Server](/dev-server-event-1-frames.png)
...
![!screenDevserver Inngest Dev Server](/event-details-1-replay.png)

### !!steps Recording the stream of events

After the event is replayed, there are two events recorded in the dashboard.

![!screenDevserver Inngest Dev Server](/dev-server-event-2.png)
...
![!screenNextjs Local app](/hello-from-next.png)
...
![!screenDevserver Inngest Dev Server](/dev-server-event-3.png)
...
![!screenDevserver Inngest Dev Server](/dev-server-event-4.png)

### !!steps Check the Inngest event log

No functions were triggered because there is not a single function in your app that is listening to such a bizarre event:

![!screenDevserver Inngest Dev Server](/dev-server-event-4-details.png)
